...
interface GlobalLoginPanelProps {
  onLoginSuccess?: (email: string, isAdmin?: boolean) => void;
}

export default function GlobalLoginPanel({ onLoginSuccess }: GlobalLoginPanelProps) {
  const { isLoginOpen, closeLogin } = useGlobalLogin();
    ...
    try {
      ...
      if (result.success) {
        ...
        if (onLoginSuccess) {
          ...
          refreshAuth();
          setMode(isAdmin ? "admin-dashboard" : "dashboard");
        }
