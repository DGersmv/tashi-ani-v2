...
async function ensureAdminUser() {
  let admin = await prisma.user.findFirst({ where: { role: 'MASTER' } });
  ...
  if (!admin) {
    ...
    console.log(`ℹ️  Используется администратор: ${admin.email}`);
  }
  return admin;
}
...
async function restoreObjects() {
  ...
  const scannedData = JSON.parse(fs.readFileSync('/tmp/scanned_files.json', 'utf-8'));
  ...
  console.log('═══════════════════════════════════════\n');
  console.log(`👤 Администратор: ${admin.email} (ID: ${admin.id})`);
}
